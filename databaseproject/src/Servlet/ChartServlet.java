...
public class ChartServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(cname + " " + syear + "\n");
		
		sql = "select * " +
	        	  "from country " +
	        	  "where cname = '" + cname + "' and year >= '" + syear + "' and year <= '" + eyear + "' and type = '" + type + "' " +
	        	  "order by year;";
		...
		try {
			...
			while (rs.next()){
				...
				list2.add(rs.getDouble(4));
				list3.add(rs.getDouble(5));
				list4.add(rs.getDouble(6));
				
			}
			
			list5.add(new String("Gold"));
			list5.add(new String("Silver"));
			list5.add(new String("Broze"));
